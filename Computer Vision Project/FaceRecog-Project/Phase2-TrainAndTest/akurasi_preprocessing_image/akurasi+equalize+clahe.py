...
for i, image_path in enumerate(train_images):
    ...
    image_gray = cv2.imread(full_image_path, cv2.IMREAD_GRAYSCALE)  # Convert the color image to grayscale
    image_equalized = cv2.equalizeHist(image_gray)
    clahe = cv2.createCLAHE()
    image_clahe_equalized = clahe.apply(image_equalized)
    detected_faces = classifier.detectMultiScale(image_clahe_equalized, scaleFactor=1.1, minNeighbors=6)
    ...
    for face_rect in detected_faces:
        x, y, w, h = face_rect
        cropped_image = image_clahe_equalized[y:y+h, x:x+w]
...
for i, image_path in enumerate(test_images):
    full_image_path = test_image_paths[i]
    image = cv2.imread(full_image_path)
    image_gray = cv2.imread(full_image_path, cv2.IMREAD_GRAYSCALE)  # Convert the color image to grayscale
    image_equalized = cv2.equalizeHist(image_gray)
    clahe = cv2.createCLAHE()
    ...
    for face_rect in detected_faces:
        x, y, w, h = face_rect
        face_image_gray = image_clahe_equalized[y:y+h, x:x+w]
