...
for i, image_path in enumerate(train_images):
    ...
    detected_faces = classifier.detectMultiScale(image, scaleFactor=1.05, minNeighbors=8, minSize=(50, 50))
    ...
    for face_rect in detected_faces:
        x, y, w, h = face_rect
        cropped_image = cv2.resize(image[y:y+h, x:x+w], (800, 600))
...
for i, image_path in enumerate(test_images):
    ...
    # Adjusting parameters for better face detection
    detected_faces = classifier.detectMultiScale(image_gray, scaleFactor=1.05, minNeighbors=8, minSize=(50, 50))
    ...
    for face_rect in detected_faces:
        x, y, w, h = face_rect
        face_image_gray = cv2.resize(image_gray[y:y+h, x:x+w], (800, 600))
