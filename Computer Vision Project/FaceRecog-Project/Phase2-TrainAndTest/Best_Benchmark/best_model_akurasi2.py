...
for i, image_path in enumerate(train_images):
    ...
    detected_faces = classifier.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=2 , minSize=(30, 30))
    ...
    for face_rect in detected_faces:
        x, y, w, h = face_rect
        cropped_image = cv2.resize(gray[y:y+h, x:x+w], (100, 100))
...
for i, image_path in enumerate(test_images):
    ...
    gray = apply_clahe(gray)                
    gray = apply_threshold(gray)          
    gray = cv2.GaussianBlur(gray, (5, 5), 2) # Convert the color image to grayscale
    detected_faces = classifier.detectMultiScale(gray, scaleFactor=1.2, minNeighbors=2 , minSize=(30, 30))
    ...
    for face_rect in detected_faces:
        x, y, w, h = face_rect
        face_image_gray = cv2.resize(gray[y:y+h, x:x+w], (100, 100))
