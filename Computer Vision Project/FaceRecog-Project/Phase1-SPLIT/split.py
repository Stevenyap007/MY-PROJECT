...
output_path = "Phase1-SPLIT\output"
...
for class_folder in class_folders:
    ...
    for image in train_images:
        ...
        dest_path = os.path.join(output_path, 'train', class_folder + '_' + image)  # Prefix with class_folder
    ...
    for image in test_images:
        src_path = os.path.join(class_path, image)
        dest_path = os.path.join(output_path, 'test', class_folder + '_' + image)  # Prefix with class_folder
        with open(src_path, 'rb') as src, open(dest_path, 'wb') as dest:
            dest.write(src.read())
